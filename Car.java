...
class Car{
	...
	//인스턴스 변수로서, 생명력은 해당 인스턴스가 소멸될 때까지 함께 한다.(해당 객체가 살아있을 때까지)

	//자동차의 부품이므로, 자동차와 생명을 같이 한다.
	//이러한 맴버변수는 초기화 하지 않으면 컴파일러가 최소한의 관여로 값을 자동초기화 시킨다.
	
	String color;//null로 초기화됨
	int price;//최소한의 값...0으로 초기화
	...
	public static void main(String[] args){
		/*Car클래스를 선언하면서, 해당 클래스가 보유한 변수인 멤버변수에
			...
			일반적인 프로그래밍 언어에서는 변수에 값이 없는 상태로
			다른 데이터와의 연산을 수행하게 되면 에러가 발생
		*/
